...
const PizzaBlock: React.FC<PizzaProps> = ({ id, title, price, imageUrl, sizes, types }) =>  {
  ...
  let cartItem = useSelector(selectCartItemById(id));
  ...
  function onClickType(index: number) {
    setActiveType(index);
  }
    ...
    <div className="pizza-block-wrapper">
      <div className="pizza-block">
        ...
        <div className="pizza-block__selector">
          <ul>
            {types.map((itemId, index) => (
              <li
                key={index}
                onClick={() => onClickType(itemId)}
          ...
          <ul>
            {sizes.map((size, id) => (
              <li key={id} onClick={() => setActiveSize(id)} className={activeSize === id ? 'active' : ''}>
                {size} см.
              </li>
            ))}
          </ul>
